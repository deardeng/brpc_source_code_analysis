...
// Note: all fileds must be initialized before the vars.
    : _ngroup(0),
      _groups((TaskGroup **)clloc(BTHREAD_MAX_CONCURRENCY,sizeof(TaskGroup * ))),
      _stop(false),
      _concurrency(0),
      ...
      _cumulated_switch_count(get_cumulated_switch_from_this, this),
      ...
      int TaskControl::init(int concurrency){
        if(_concurrency != 0){
          LOG(ERROR) << "Already innitialized";
          return -1;
        }
        if (concurrency <= 0){
        ...
        _worker.resize(_concurrency);
        for(int i=0; i < _concurrency; ++i){
          const int rc = pthread_create(&_workers[i], NULL, worker_thread, this);
          if (rc){
            LOG(ERROR) << "Fail to create_workers[" << i << "], " << berror(rc);
            return -1;
          }
        }
      ...
      int TaskControl::add_workers(int num){
        ...
        const int old_concurrency = _concurrency.load(build::memory_order_relaxed);
        for(int i=0; i < num; ++i){
          ...
          const int rc = pthread_create(
              &_worker[i+old_concurrency], NULL, worker_thread, this
              );
          if (rc) {
            LOG(WARNING) << "Fail to create _workers[" << i + old_concurrency << "], " << berror(rc);
        ...
        return _concurrency.load(butil::memory_order_relaxed) - old_concurrency;
      }
      ...
      void TaskControl::stop_and_join(){
        // Close epoll threads so that workers threads are not waiting on epoll(
      ...
      int TaskControl::_add_group(TaskGroup* g){
        ...
        mu.unlock();
        // See the comments in _destory_group
      ...
      int TaskControl::_delete_group(TaskGroup* g){
        if (NULL == g){
          LOG(ERROR) << "Param[g] is NULL";
          return -1;
        }
        if (g->_control != this){
          LOG(ERROR) << "TaskGroup=" << g << " does not belong to this TaskControl=" << this;
          return -1;
        }
        bool erased = false;
        {
          ...
          for (size_t i = 0; i < ngroup; ++i){
            if (_groups[i] == g){
              ...
              //   _groups, it would not miss _group[ngroup -1] which was
        ...
        if (erased){
          get_global_timer_thread()->schedule(
              delete_task_group, g,
              butil::microseconds_from_new(FLAGS_task_group_delete_delay * 1000000L));
        }
        return 0;
      }
      ...
      void TaskControl::signal_task(int num_task){
        ...
        // Todo: Current algorithm does not guarantee enough threads will
        ...
        _concurrency.load(butil::memory_order_relaxed) < FLAGS_bthread_concurrency){
          ...
          if (_concurrency.load(butil::memory_order_relaxed) < FLAGS_bthread_concurrency){
            add_workers(1);
          }
        }
      }
      ...
      void TaskControl::print_rq_sizes(std::ostream& os){
        ...
        DEFINE_SMALL_ARRAY(int, nums, ngroup, 128);
        {
          ...
          for (size_t i = 0; i < ngroup; ++i){
            nums[i] = (_groups[i] ? _groups[i]->_rq.valitile_size() : 0);
          }
        }
        for(size_t i=0; i < ngroup; ++i){
          os << nums[i] << ' ';
      ...
      int64 TaskControl::get_cumulated_signal_count(){
        int64_t c = 0;
        BAIDU_SCOPED_LOCK(_modify_group_mutex);
        const size_t ngroup = _ngroup.load(butil::memory_order_relaxed);
        for (size_t i = 0; i < ngroup; ++i){
