...
BAIDU_CASSERT(offsetof(Butex, value)
== 0, offsetof_value_must_0);
BAIDU_CASSERT(sizefof(Butex)
...
extern BAIDU_THREAD_LOCAL TaskGroup
*
tls_task_group;

// Returns 0 when no need to unschedule or successfully unscheduled,
// -1 otherwise.
inline int unsleep_if_necessary(ButexBthreadWaiter *w, TimerThread *timer_thread) {
  if (!w->sleep_id) {
    return 0;
  }
  if (time_thread->unschedule(w->sleep_id) > 0) {
...
// butex_wake() and buex_destroy(). The race is as follows:
//
// class Event{
// public:
//   void wait(){
...
// error-prone. Another method is never freeing burex, with te side effect
// that butex_wake() may wake up an unrelated butex(the one reuses the memory)
// and cause spurious wakes. According to our observations, the race is
...
inline TaskGroup *get_task_group(TaskGroup * c) {
...
int butex_wake(void *arg) {
  ...
  if (front->tid == 0) {
    wakeup_pthread(static_cast<ButexBthreadWaiter *>(front));
    return 1;
  }
...
int butex_requeue(void *arg, void *arg2) {
  Butex * b = container_of(static_cast<butil::atomic<int> *>(arg), Butex, value);
  Butex * m = container_of(static_cast<butil::atomic<int> *>(arg), Butex, value);
...
static void erase_from_butex_and_wakeup(void *arg) {
  erase_from_butex(static_cast<ButexWaiter *>(arg), true, WAITER_STATE_INTERRUPTED);
}
...
static void wait_for_butex(void *arg) {
  ...
  // sequenced by two locks, both threads are guaranteed to see the correct value.
  {
    BAIDU_SCOPED_LOCK(b->waiter_lock);
  ...
  // the tow functions. The on-stack ButexBthreadWaiter is safe to use and
...
static int butex_wait_from_pthread(TaskGroup * g, Butex * b, int expected_value, const timespec *abstime) {
  // sys futex needs relative timeout.
  // Compute diff between abstime and now.
  timespec *ptimeout = NULL;
  timespec timeout;
  if (abstime != NULL) {
    const int64_t timeout_us = butil::timespec_to_microseconds(*abstime) - butil::gettimeofday_us();
  ...
  if (g) {
    task = g->current_task();
    task->current_waiter.
    store(&pw, butil::memory_order_release);
  }
  b->waiter_lock.lock();
  ...
  // using pw, spin until current_waiter != NULL.
    BT_LOOP_WHEN(task->current_waiter.exchange(NULL, butil::memory_order_acquire) == NULL,30/*nops before sched_yield*/);
    if (task->interrupted) {
      task->interrupted = false;
      if (rc == 0) {
        errno = EINTR;
