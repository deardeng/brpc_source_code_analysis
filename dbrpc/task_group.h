//
// Created by dengxin05 on 2020/6/19.
//
...
// For exiting a bthread.
 ...
 class TaskGroup{
  public:
   ...
   static int start_foregroup(TaskGroup** pg, bthread_t* __restrict tid,
   ...
   //   Called from worker: start_backgroup<false>
   //   Called from non-worker: start_backgroud<true>
   // Return 0 on success, errno otherwise.
   ...
   void set_remained(RemainedFn cb, void* arg){
     _last_contex_remained = cb;
     _last_contex_remained_arg = arg;
   }
   ...
   // just the result at this very moment which may change sool.
   ...
   TaskMeta* current_task() const{
     return _cur_meta;
   }
   bthread_t current_tid() const {
     return _cur_mata->tid;
   }
   ...
   void flush_nosignal_task_remote_locked(butil::Mutex& locked_mutex);
  ...
  private:
   ...
   static void _release_last_contex(void*);
