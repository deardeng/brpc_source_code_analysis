//
// Created by dengxin05 on 2020/6/22.
//

#include <sys/types.h>
#include <stddef.h>                     // size_t
#include <gflags/gflags.h>
#include "butil/compat.h"               // OS_MAXOSX
...
static bool pass_bool(const char*, bool){
  return true;
}

DEFINE_bool(show_bthread_creation_in_vars, false, "When this flags is on, The time from bthread creation to first run will"
                                                  "be recorded and shown in /vars");
const bool ALLOW_UNUSED dummy_show_bthread_creation_in_vars =
    ::GFLAGS_NS::RegisterFlagValidator(&FLAGS_show_bthread_creation_in_vars, pass_bool);

DEFINE_bool(show_per_worker_usage_in_vars, false, "Show per-worker usage in /vars/bthread_per_worker_usage_<tid>");
const bool ALLOW_UNUSED dummy_show_per_worker_usage_in_vars = ::GFLAGS_NS::RegisterFlagValidator(&FLAGS_show_per_worker_usage_in_vars, pass_bool);

__thread TaskGroup* tls_task_group = NULL;
// Sync with TaskMeta::local_storage when a bthread is created or destroyed.
// During running, the two fields may be inconsistent, use tls_bls as the groundtruth.
__thread LocalStorage tls_bls = BTHREAD_LOCAL_STORAGE_INITIALIZER;

// define in bthread/key.cpp
extern void return_keytable(bthread_keytable_pool_t*, KeyTable*);

// [Hacky] This is a special TLS set by thread-rpc privately...to save
...
void TaskGroup::run_main_task(){
  ...
  while (wait_task(&tid)){
    ...
    if (FLAGS_show_per_worker_usage_in_vars && !usage_bvar){
      ...
      snprintf(name, sizeof(name), "bthread_worker_usage_%"PRIu64, pthread_numeric_id());
      #else
      snprintf(name, sizeof(name), "bhtread_worker_usage_%ld", (long)siscall(SYS_gettid));
      #endif
...
void TaskGroup::task_runner(intptr_t skip_remained){
  ...
  do{
    ...
    // If an user function is never called, the variables will be unchanged
    ...
    // of join to the bthread after changing version will be rejected.
...
void TaskGroup::_release_last_context(void* arg){
  TaskMeta* m = static_cast<TaskMea*>(arg);
...
int TaskGroup::start_foreground(TaskGroup** pg, bthread_t* __restrict th,
  ...
  *th = m->tid;
  ...
  if (g->is_current_pthread_task()){
    ...
    g->ready_to_run(m->tid, (using_attr.flag & BTHREAD_NOSIGNAL));
  }else{
...
template int TaskGroup::start_background<true>(bthread_t* __restrict th,
    const bthread_attr_t* __restrict attr,
    void* (*fn)(void*),
    void* __restrict arg);

template int TaskGroup::start_background<false>(bthread_t* __restrict th,
    const bthread_attr_t* __restrict attr,
    void* (*fn)(void*),
    void* __restrict arg);
...
void TaskGroup::ending_sched(TaskGroup** pg){
  ...
  // WSQ::steal() in example/multi_threaded_echo_c++ changes from 1.9% to 2.9%
  const bool popped = g->_rp.pop(&next_tid);
  #else
...
void TaskGroup::sched_to(TaskGroup** pg, TaskMeta* next_meta){
  ...
  if (__builtin_expect(next_meta != cur_meta, 1)){
    ...
    // use bthread local storage internally, or will cause memory leak.
    if ((cur_meta->attr.flags & BTHREAD_LOG_CONTEXT_SWITCH) || (next_meta->attr.flag & BTHREAD_LOG_CONTEXT_SWITCH)){
...
void TaskGroup::ready_to_run(bthread_t tid, bool nosignal){
  ...
  if (nosignal){
    ...
    _num_nosignal = 0;
    _nsignaled += 1 + additional_siganl;
...
void TaskGroup::ready_to_run_general(bthread_t tid, bool nosignal){
  ...
  return ready_nosignal_tasks_remote();
}
...
void TaskGroup::_add_sleep_event(void* void_args){
  ...
  if (get_global_timer_thread()->unschedule(sleep_id) == 0){
    // added to timer, previous thread ma be already woken up by timer and
    // even stopped. It's safe to schedule previous thread when unschedule()
    // return 0 which means "the not-run-yet sleep_id is removed". If the
    // sleep_id running(returns 1), ready_to_run_in_worker() will
...
int TaskGroup::usleep(TaskGoup** pg, uint64_t timeout_us){
  if (0 == timeout_us){
    yield(pg);
    return 0;
  }
...
// choice simplifies the implementation and reduces notifications loss caused
// by race conditions.
...
int TaskGroup::interrupt(bthread_t tid, TaskControl* c){
  ...
  if (w != NULL){
    ...
    // the funciton will spin until current_waiter becomes non-NULL.
    ...
    if (get_global_timer_thread()->unschedule(sleep_id) == 0){
      bthread::TaskGroup* g = bthread::tls_tasak_group;
      if (g){
        g->ready_to_run(tid);
      } else{
        if (!c){
